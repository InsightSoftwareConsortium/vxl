...
inclusion with `v3p_netlib.h`:
    ```bash
    for d in blas linpack temperton eispack laso arpack napack minpack opt linalg toms datapac; do
      for f in ${d}/*.f; do
        b=`echo "$f" | sed 's/.f$//'`
        if [ ! -f "${b}.c" ]; then
...
with the mangled interface.  Include them all in `v3p_netlib_prototypes.h`.
    ...
    for f in blas/*.P linpack/*.P temperton/*.P eispack/*.P laso/*.P arpack/*.P napack/*.P minpack/*.P opt/*.P linalg/*.P toms/*.P datapac/*.P; do
      b=`echo "$f" | sed 's/.P//'`
      if [ ! -f "${b}.h" ] ; then
