...
test_operators()
{
  const vnl_quaternion<double> a(0, 0, 0, 1);
  const vnl_quaternion<double> b(2, 2, 2, 2);
  const vnl_quaternion<double> d(2, 2, 2, 3);
  vnl_quaternion<double> e(1, 2, 3, 4);
  TEST("!=", a != b, true);
  TEST("==", a == a, true);
  const vnl_quaternion<double> c = a + b;
  TEST("+", c, d);
  TEST(".x()", e.x(), 1.0);
  TEST(".y()", e.y(), 2.0);
  TEST(".z()", e.z(), 3.0);
...
test_rotation_matrix_and_euler_angles()
{
  ...
  for (unsigned i = 0; i < ntrials; ++i)
  {
    ...
    const double x = rng.drand32(-1.0, 1.0);
    const double y = rng.drand32(-1.0, 1.0);
    ...
    const vnl_quaternion<double> q(axis, ang);
    vnl_vector_fixed<double, 3> eu = q.rotation_euler_angles();
    const vnl_matrix_fixed<double, 3, 3> R = (q.rotation_matrix_transpose()).transpose();

    // Use vnl_rotation_matrix() with axis+angle form
    {
      const vnl_vector_fixed<double, 3> axis_ang = axis * ang;
      const vnl_matrix_fixed<double, 3, 3> M = vnl_rotation_matrix(axis_ang);
      const vnl_matrix_fixed<double, 3, 3> D = R - M;
    ...
    // Use vnl_rotation_matrix() with euler angles.
    {
      ...
      ey *= eu[1];
      ez *= eu[2];
      const vnl_matrix<double> Rx = vnl_rotation_matrix(ex);
      const vnl_matrix<double> Ry = vnl_rotation_matrix(ey);
      const vnl_matrix<double> Rz = vnl_rotation_matrix(ez);
      const vnl_matrix<double> M = Rz * Ry * Rx;
...
test_rotations()
{
  const vnl_vector_fixed<double, 3> p1(2, 2, 2);
  const vnl_vector_fixed<double, 3> p2(1, 0, 0);
  const vnl_vector_fixed<double, 3> p3(0, 1, 0);
  const vnl_vector_fixed<double, 3> e0(0, 0, 0);
  const vnl_quaternion<double> q0(0, 0, 0, 0);
  ...
  const vnl_quaternion<double> q0_b(0, 0, 0, 1);
  ...
  const vnl_quaternion<double> q0_c(0, 0, 0, -4);
  ...
  const vnl_quaternion<double> q0_d(0, 0, 0);
  ...
  const vnl_quaternion<double> q1_c = -q1;
  const vnl_vector_fixed<double, 3> e1_c = q1_c.rotation_euler_angles();
  TEST_NEAR("-q1 -> Euler angles", vnl_vector_ssd(e1_c, e1), 0.0, 1e-8);

  std::cout << "q1 -> Euler angles: " << e1 << std::endl;
  const vnl_quaternion<double> q1_b(e1(0), e1(1), e1(2));
